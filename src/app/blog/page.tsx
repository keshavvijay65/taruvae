...
function BlogContent() {
    ...
    useEffect(() => {
        // Load default posts immediately for fast initial render
        const defaultPosts = getDefaultBlogPosts();
        const publishedDefaultPosts = defaultPosts.filter(post => post.published).sort((a, b) => b.publishedAt - a.publishedAt);
        setPosts(publishedDefaultPosts);
        setLoading(false);
        ...
        const loadPosts = async () => {
            try {
                ...
                // Keep default posts already set
            }
        };
        ...
        const unsubscribe = subscribeToBlogPosts((updatedPosts) => {
            ...
            updateTimeout = setTimeout(() => {
                if (updatedPosts && Array.isArray(updatedPosts) && updatedPosts.length > 0) {
    ...
    return (
        <div className="min-h-screen rich-gradient">
                        ...
                        <div className="flex gap-3 flex-wrap justify-center md:justify-start">
                            {categories.map((category) => (
                                <button
                                    ...
                                    className={`px-5 py-2.5 rounded-xl font-semibold text-sm transition-all duration-200 shadow-sm ${selectedCategory === category
            ...
            {/* Blog Posts Grid */}
            <div className="container mx-auto px-6 md:px-8 lg:px-10 max-w-7xl pt-[calc(var(--header-height,80px)+2rem)] pb-10">
