...
const getStatusProgress = (status: OrderStatus) => {
    const statuses: OrderStatus[] = ['pending', 'processing', 'shipped', 'out_for_delivery', 'delivered'];
...
function OrdersContent() {
    ...
    }, [isAuthenticated, authLoading, user?.uid, user?.email]);

    // Check for success redirect and show invoice
    useEffect(() => {
        if (searchParams.get('success') === 'true' && searchParams.get('orderId') && orders.length > 0) {
            const orderId = searchParams.get('orderId');
            const foundOrder = orders.find(o => o.orderId === orderId);
            if (foundOrder) {
                setShowInvoice(foundOrder);
                // Clear query params without refresh if possible, or just keep them
            }
        }
    }, [searchParams, orders]);
    ...
    const loadOrders = async () => {
        ...
        try {
            ...
            try {
                ...
                const firebasePromise = getAllOrdersFromFirebase(user?.uid, user?.email);
        ...
        <div className="min-h-screen bg-gradient-to-b from-brand-50 to-white">
            ...
            <div className="container mx-auto px-6 md:px-8 lg:px-10 max-w-7xl pt-[calc(var(--header-height,80px)+2rem)] pb-8 md:pb-12">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <Link
                        href="/"
                        className="bg-brand text-white px-6 py-2 rounded-lg font-semibold hover:bg-brand-dark transition-colors"
                    >
                        Continue Shopping
                    </Link>
                </div>
                                ...
                                >
                                        ...
                                        <div className="relative">
                                            <div className="flex items-center justify-between mb-2">
                                                {['Pending', 'Processing', 'Shipped', 'Out for Delivery', 'Delivered'].map((step, index) => {
                                                    const stepStatus: OrderStatus[] = ['pending', 'processing', 'shipped', 'out_for_delivery', 'delivered'];
                                            ...
                                            <div className="relative h-1 bg-gray-200 rounded-full -mt-8 mb-8">
                                                <div
                                                    className="absolute top-0 left-0 h-full bg-brand-dark rounded-full transition-all duration-500"
                                    ...
                                    <div className="flex flex-wrap gap-3 pt-4 border-t border-gray-200">
                                        ...
                                        <Link
                                            ...
                                            className="px-4 py-2 bg-white border-2 border-brand-dark text-brand-dark rounded-lg text-sm font-semibold hover:bg-soft transition-colors"
                                        >
                                            Track Order
                                        </Link>
                                        <button
                                        ...
                                        <div className="mt-6 pt-6 border-t border-gray-200 space-y-6">
                                            ...
                                            <div className="bg-gray-50 rounded-lg p-4">
                                                ...
                                                <p className="text-sm text-gray-600 mt-2">
                                                    Payment Method: <span className="font-semibold">{(order.paymentMethod?.toLowerCase() === 'cod') ? 'Cash on Delivery' : 'Online Payment'}</span>
                                                </p>
                                            </div>
                                        </div>
                                    )}
