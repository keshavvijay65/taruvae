        ...
        const initializePage = async () => {
            try {
                ...
                if (auth === 'true') {
                    ...
                    // STEP 1: Load cached orders IMMEDIATELY
                    try {
                        ...
                        try {
                            const firebaseOrders = await Promise.race([
                                ...
                                new Promise<Order[]>((_, reject) =>
                                    setTimeout(() => reject(new Error('Orders load timeout')), 2000) // Reduced to 2s
                                )
                            ]).catch((error) => {
                                console.warn('Orders load timeout, using cache:', error);
                    ...
                    // STEP 3: Subscribe to real-time updates (non-blocking)
                    try {
                    ...
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <div className="flex gap-2">
                            <select
                                ...
                                className="px-4 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-dark"
                            >
                                ...
                                <tbody>
                                    {filteredOrders.slice(0, 10).map((order) => (
                                        <tr key={order.orderId} className="border-b border-gray-100 hover:bg-gray-50">
                                            ...
                                            <td className="py-3 px-4 font-bold text-brand-dark">₹{order.total}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
